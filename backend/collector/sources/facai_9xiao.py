...
PRED_RE = re.compile(rf"{re.escape(TITLE)}\s*[:：]?\s*[〖【\[［]\s*([^〗】\]］]+?)\s*[〗】\]］]")
CLAIM_RE = re.compile(r"(?:开奖?|開獎?)\s*[:：]?\s*(.+)$")


def extract(raw: str) -> list[tuple[str, str, list[str], dict[str, str | None]]]:
    """Extract all nine published positions without silently removing repeats."""
    ...
    for line in html_to_lines(raw):
        ...
        if period_match is None or pred_match is None:
            continue
        body = pred_match.group(1)
        xiaos = [char for char in body if char in XIAO]
        if len(xiaos) != 9:
            continue
        claim_match = CLAIM_RE.search(line)
        claimed = parse_claimed(claim_match.group(1) if claim_match else "")
        rows.append((period_match.group(1), line, xiaos, claimed))
    return rows
...
def build(lottery: str, period: str | None, fixture: str | None) -> PredV1:
    ...
    for period_raw, raw, xiaos, claimed in extract(page.text):
        try:
        ...
        source_text = "".join(xiaos)
        items.append(
            PredItem(
                period_raw=period_raw,
                period=normalized_period,
                published_at=None,
                preds=[PredAtom(kind="xiao", value=value, text=source_text) for value in xiaos],
                claimed=Claimed(
    ...
    if not items:
        raise ValueError(f"{SOURCE_NAME} 未找到恰好9个生肖位置的预测")
    return PredV1(
        ok=True,
        schema_name="pred.v1",
        source_id=SOURCE_ID,
