...
PRED_RE = re.compile(rf"{re.escape(TITLE)}\s*[】\]]?\s*[〖【\[]\s*([单双])\s*数\s*[〗】\]]")
CLAIM_RE = re.compile(r"(?:开奖?|開獎?)\s*[:：]?\s*(.+)$")


def extract(raw: str) -> list[tuple[str, str, str, dict[str, str | None]]]:
    """Keep only rows that publish an explicit 单数 or 双数 prediction."""
    ...
    for line in html_to_lines(raw):
        ...
        if period_match is None or pred_match is None:
            # Values such as “？数” are future placeholders, not predictions.
            continue
...
def build(lottery: str, period: str | None, fixture: str | None) -> PredV1:
    ...
    for period_raw, raw, odd_value, claimed in extract(page.text):
        try:
        ...
        items.append(
            PredItem(
                ...
                published_at=None,
                preds=[PredAtom(kind="odd", value=odd_value, text=f"{odd_value}数")],
                claimed=Claimed(
    ...
    if not items:
        raise ValueError(f"{TITLE} 未找到明确的单数或双数预测")
    return PredV1(
        ok=True,
        schema_name="pred.v1",
        source_id=SOURCE_ID,
