...
SOURCE_ID = "zhaocai_jinbao"
SOURCE_NAME = "招彩金宝"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "texiao"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1780230465222")

HEADER_RE = re.compile(r"^\s*第?\s*(?P<period>\d{1,7})\s*期.*招彩金宝")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for match, block in period_blocks(raw, HEADER_RE):
        ...
        if len(xiaos) != 3 or len(nums) != 9:
            continue
        atoms = [
            {"kind": "xiao", "value": atom["value"], "text": xiao_line}
            for atom in xiaos
        ] + [
            {"kind": "num", "value": atom["value"], "text": number_line}
            for atom in nums
        ]
        rows.append(
            ParsedRow(
                period_raw=match.group("period"), preds=atoms,
                claimed=claimed_from(" ".join(block)), raw_text=" ".join(block[:4]),
            )
        )
    return rows
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未找到完整的三肖九码",
    )
