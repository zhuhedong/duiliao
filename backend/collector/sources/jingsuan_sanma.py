...
SOURCE_ID = "jingsuan_sanma"
SOURCE_NAME = "精算三码"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "tema_n"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1789991780509")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for i, ln in enumerate(lines):
        m = re.match(r"^(\d+)\s*期\s*精算三码", ln)
        if not m:
            continue
        period = m.group(1)
        block = [ln]
        for j in range(i + 1, min(i + 8, len(lines))):
            if re.search(r"^\d+\s*期", lines[j]):
                break
            block.append(lines[j])
        chunk = " ".join(block)
        num_line = next((l for l in block if "【" in l), block[1] if len(block) > 1 else "")
        nums = atoms_num(num_line)
        if not nums or "88" in num_line or "發" in ln:
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未提取到有效精算三码预测数据",
    )
