...
SOURCE_ID = "sima_shuju"
SOURCE_NAME = "研究所数据四码"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "tema_n"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1789908353019")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for i, ln in enumerate(lines):
        m = re.match(r"^(\d+)\s*期数", ln)
        if not m:
            continue
        period = m.group(1)
        block = [ln]
        for j in range(i + 1, min(i + 5, len(lines))):
            if re.search(r"^\d+\s*期数", lines[j]) or "添加迅聊" in lines[j]:
                break
            block.append(lines[j])
        chunk = " ".join(block)

        nums = atoms_num(block[1] if len(block) > 1 else "")
        if all(n["value"] == "88" for n in nums) or "？00" in chunk:
            ...
            claimed_line = next((l for l in block if "开奖结果" in l), "")
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未提取到有效四码预测数据",
    )
