...
from dj_util import atoms_num, atoms_xiao, period_blocks, urls_for

SOURCE_ID = "vip_1xiao6ma"
SOURCE_NAME = "内部vip一肖六码"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "texiao"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1788351048680")

HEADER_RE = re.compile(r"^\s*(?P<period>\d{1,7})\s*期\s*【一肖六码】")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for match, block in period_blocks(raw, HEADER_RE):
        if len(block) < 3:
            continue
        xiao_line, number_line = block[1], block[2]
        xiaos = atoms_xiao(xiao_line)
        nums = atoms_num(number_line)
        if len(xiaos) != 1 or len(nums) != 6:
            continue
        ...
        claimed_text = block[0].split("】", 1)[1].lstrip(" :：") if "】" in block[0] else ""
        rows.append(
            ParsedRow(
                period_raw=match.group("period"), preds=atoms,
                claimed=parse_claimed(claimed_text), raw_text=" ".join(block[:3]),
            )
        )
    return rows
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未找到完整的一肖六码",
    )
