...
import re
from dj_util import urls_for, html_to_lines, twoface, claimed_of
...
def extract(raw):
    ...
    for line in html_to_lines(raw):
        if not re.search(r"\d+\s*期", line):
            continue
        if not (("小单" in line or "大单" in line) and ("小双" in line or "大双" in line)):
            continue
        if "海哥" in line or "⑨肖" in line:
            continue
        m=re.search(r"(\d+)\s*期", line)
        rows.append((m.group(1), line, twoface(line), claimed_of(line)))
    return rows

SOURCE_ID = 'sange_danshuang'
SOURCE_NAME = '三个单双'
