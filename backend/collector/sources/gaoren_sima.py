...
SOURCE_ID = "gaoren_sima"
SOURCE_NAME = "高人四码+一肖"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "texiao"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1789460787496")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for i, ln in enumerate(lines):
        m = re.search(r"(\d+)期", ln)
        if not m or not ("开" in ln or "准" in ln or "中" in ln or "期" in ln):
            continue
        period = m.group(1)
        block = [ln]
        ...
        for b in block:
            if "码" in b:
                nums.extend(atoms_num(b))
            if "肖" in b:
                xiaos.extend(atoms_xiao(b))
        atoms = xiaos + nums
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未提取到有效四码+一肖预测数据",
    )
