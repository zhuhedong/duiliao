...
SOURCE_ID = "tianxuan_zhiwang"
SOURCE_NAME = "天选之王一肖一码"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "texiao"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1789557830644")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for i, ln in enumerate(lines):
        ...
        body_lines = [
            l
            for l in block[1:]
            if not re.search(r"期|开奖|开:|開獎|com|大师|实力|车", l)
        ]
        body_text = " ".join(body_lines)
        xiaos = atoms_xiao(body_text)
        nums = atoms_num(body_text)
        atoms = xiaos + nums
        claimed_line = next((l for l in block if re.search(r"开奖|开:", l)), "")
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未提取到有效一肖一码预测数据",
    )
