...
import re
from dj_util import urls_for, html_to_lines, atoms_xiao, claimed_of
URLS = urls_for("/api/v1/index/config/byid/1788608575932")
def extract(raw):
    lines=html_to_lines(raw)
    rows=[]
    for i,ln in enumerate(lines):
        if ln.strip()=="二肖中特" and i>=1:
            period=None
            for back in lines[max(0,i-3):i]:
                m=re.fullmatch(r"(\d{3,4})期?", back.replace("第",""))
                if m:
                    period=re.sub(r"\D","", m.group(1) if False else back)
                    period=re.sub(r"\D","", back)
                    break
            pair = lines[i+1] if i+1<len(lines) else ""
            chunk=" ".join(lines[max(0,i-2):i+5])
            if period:
                rows.append((period, chunk, atoms_xiao(pair), claimed_of(chunk)))
    return rows

SOURCE_ID = 'xiemi_2xiao'
SOURCE_NAME = '泄密二肖'
