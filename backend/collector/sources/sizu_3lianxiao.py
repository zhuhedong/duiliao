...
SOURCE_ID = "sizu_3lianxiao"
SOURCE_NAME = "前程似锦四组3连肖"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "lianxiao_n"
HIT_MODE = "all"
URLS = urls_for("/api/v1/index/config/byid/1690557551507")

HEADER_RE = re.compile(r"^\s*(?P<period>\d{1,7})\s*期\s*【四组3连肖】")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for match, block in period_blocks(raw, HEADER_RE):
        ...
        for content in BRACKET_RE.findall(" ".join(block[1:])):
            values = [atom["value"] for atom in atoms_xiao(content)]
            if len(values) == 3:
                groups.append((content, values))
        if len(groups) != 4:
            continue
        for index, (evidence, values) in enumerate(groups, start=1):
            rows.append(
                ParsedRow(
                    period_raw=match.group("period"), group_key=str(index),
                    preds=[{"kind": "xiao", "value": value, "text": evidence} for value in values],
                    claimed=claimed_from(block[0]),
                    raw_text=f"{block[0]} 第{index}组【{evidence}】",
                )
            )
    return rows
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未找到四组完整且相互独立的三连肖",
    )
