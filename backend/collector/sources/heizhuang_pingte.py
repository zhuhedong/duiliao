...
SOURCE_ID = 'heizhuang_pingte'
SOURCE_NAME = '黑庄克星平特'
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = 'pingte_xiao'
HIT_MODE = 'any'
TITLE = '黑庄克星'
TITLE_ALIASES = (TITLE, '黑庄平特')
KIND = 'xiao'
...
def build(lottery: str, period: str | None, fixture: str | None) -> PredV1:
    ...
    column_text = isolate_title(page.text, TITLE, extra_ok=('黑庄平特',))
    if not column_text:
    ...
    for row in rows:
        ...
        if not any(title in raw for title in TITLE_ALIASES):
            continue
        try:
