...
SOURCE_ID = "hengsao_liuhe"
SOURCE_NAME = "横扫六合"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "texiao"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1780230316883")

HEADER_RE = re.compile(r"^\s*(?P<period>\d{1,7})\s*期.*横扫六合")
GROUP_RE = re.compile(rf"([{XIAO}])\s*-\s*(\d{{1,2}})\s*-\s*(\d{{1,2}})\s*-\s*(\d{{1,2}})")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for match, block in period_blocks(raw, HEADER_RE):
        ...
        if len(groups) != 2 or any(not 1 <= int(num) <= 49 for group in groups for num in group[1:]):
            continue
        atoms: list[dict[str, str]] = []
        for xiao, *nums in groups:
            evidence = f"{xiao}-" + "-".join(f"{int(num):02d}" for num in nums)
            atoms.append({"kind": "xiao", "value": xiao, "text": evidence})
            atoms.extend({"kind": "num", "value": f"{int(num):02d}", "text": evidence} for num in nums)
        rows.append(
            ParsedRow(
                period_raw=match.group("period"), preds=atoms,
                claimed=claimed_from(block[0]), raw_text=" ".join(block),
            )
        )
    return rows
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未找到完整的两组一肖三码",
    )
