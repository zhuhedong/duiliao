...
HEADER_RE = re.compile(r"^\s*第?\s*(?P<period>\d{1,7})\s*期\s*[【〖]6肖18码[】〗]")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for match, block in period_blocks(raw, HEADER_RE):
        ...
        for content in BRACKET_RE.findall(" ".join(block[1:])):
            xiaos = atoms_xiao(content)
            nums = atoms_num(content)
            if len(xiaos) == 1 and len(nums) == 3:
                groups.append((content, xiaos[0]["value"], nums))
        if len(groups) != 6:
            continue
        atoms: list[dict[str, str]] = []
        for evidence, xiao, nums in groups:
            atoms.append({"kind": "xiao", "value": xiao, "text": evidence})
            atoms.extend({"kind": "num", "value": atom["value"], "text": evidence} for atom in nums)
        rows.append(
            ParsedRow(
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未找到完整的六组一肖三码",
    )
