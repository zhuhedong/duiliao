...
from dj_util import atoms_xiao, period_blocks, urls_for

SOURCE_ID = "guancha_1xiao"
SOURCE_NAME = "数据观察员一肖"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "texiao"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1788436186186")

HEADER_RE = re.compile(r"^\s*(?P<period>\d{1,7})\s*期\s*[:：]\s*【主攻一肖】")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for match, block in period_blocks(raw, HEADER_RE):
        ...
        if len(candidates) != 1:
            continue
        claimed_raw = "中奖" if any("中奖" in line for line in block[2:]) else None
        rows.append(
            ParsedRow(
                period_raw=match.group("period"),
                preds=[{"kind": "xiao", "value": candidates[0]["value"], "text": evidence}],
                claimed={"status": "unknown", "xiao": None, "num": None, "raw": claimed_raw},
                raw_text=" ".join(block),
            )
        )
    return rows
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未找到一期恰好一个有效特肖",
    )
