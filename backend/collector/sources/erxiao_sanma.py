...
SOURCE_ID = "erxiao_sanma"
SOURCE_NAME = "二肖三码精研特供"
SITE_FAMILY = "dingjian_dashi"
PLAY_TYPE = "texiao"
HIT_MODE = "any"
URLS = urls_for("/api/v1/index/config/byid/1789461263310")
...
def extract(raw: str) -> list[ParsedRow]:
    ...
    for i, ln in enumerate(lines):
        ...
        chunk = " ".join(block)
        xiao_line = next((l for l in block if "主攻" in l), "")
        num_line = next((l for l in block if "重点" in l), "")
...
def build(lottery: str, period: str | None, fixture: str | None):
    ...
    return build_prediction(
        ...
        empty_error=f"{SOURCE_NAME} 未提取到有效二肖三码预测数据",
    )
