...
import re
from dj_util import urls_for, html_to_lines, atoms_xiao, atoms_num, claimed_of
URLS = urls_for("/api/v1/index/config/byid/1788521444713")
def extract(raw):
    lines=html_to_lines(raw); rows=[]
    for i,ln in enumerate(lines):
        m=re.match(r"第?(\d+)期$", ln.replace(" ",""))
        if m:
            body=lines[i+1] if i+1<len(lines) else ""
            chunk=" ".join(lines[i:i+4])
            rows.append((m.group(1), chunk, atoms_xiao(body)+atoms_num(body), claimed_of(chunk)))
    return rows

SOURCE_ID = 'yixiao_erma'
SOURCE_NAME = '顶尖高手壹肖贰码'
