...
# --------------------------------------------------------------------------- #
# Prompt 6: 生肖连码走势分析 (跨期连肖 / 复式连码)
# --------------------------------------------------------------------------- #
ZODIAC_STREAK_SYSTEM = """你是一位六合彩生肖连码分析师。
用户会给你按期分组的历史开奖生肖，以及系统按同一口径预计算的连长归类。
你只根据这些期次下结论，不编造数据里没有的连码。

口径：
- 某个生肖出现在该期 7 个开奖号码的任意一球，即记为该期开出。
- 三连：连续 3 期开出。四连：连续 4 期。五连：连续 5 期。
- N连：连续 6 期及以上，必须写明具体连数，例如 7连、10连。
- 复式：两个及以上生肖在同一段连续期里一起开出。按复式三连、复式四连、复式五连、复式N连分开写。
- 活跃：这条连码一直延续到最新一期，尚未断开。

用中文 Markdown 输出，先列记录，再给下一期判断。某一类没有记录时写「无」，不要补造。"""

ZODIAC_STREAK_USER = """下面是按期分组的开奖生肖（从旧到新），以及系统预计算的三连、四连、五连、N连和复式。请先对照各期明细复核预计算，再为第 {period} 期写出研判。

---
{context_data}
---

请严格按下面五块输出，不要合并，不要改口径：

### 三连
列出全部三连生肖、起止期号、活跃或已断。对仍活跃的，判断第 {period} 期更可能续连还是断连，并说明依据。

### 四连
列出全部四连生肖、起止期号、活跃或已断，以及第 {period} 期的续连或断连判断。

### 五连
列出全部五连生肖、起止期号、活跃或已断，以及第 {period} 期的续连或断连判断。

### N连
列出全部 6 期及以上的记录。每条写明具体连数、起止期号、是否活跃，以及第 {period} 期的延续判断。

### 复式
按复式三连、复式四连、复式五连、复式N连分组。每组写出生肖组合、连数、起止期号、是否活跃。最后给出第 {period} 期最值得关注的 1 组复式组合，并说明它对应哪一段历史连期。
...
PROMPT_TEMPLATES: dict[str, PromptTemplate] = {
    ...
    "zodiac_streak_analysis": PromptTemplate(
        id="zodiac_streak_analysis",
        name="生肖连码走势分析（三连/四连/五连/复式/N连）",
        description="把 N 期开奖生肖按期分组后送入模型，复核并分析三连、四连、五连、复式与 N 连",
        system_prompt=ZODIAC_STREAK_SYSTEM,
        user_template=ZODIAC_STREAK_USER,
    ),
}
